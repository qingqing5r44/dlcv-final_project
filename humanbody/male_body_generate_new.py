...
sys.path.append('F:\project\splocs-master')
C_body = np.load('E:\\reconstruction\\250\\work\\C_body_data.npz')['C']
gmm = joblib.load('gmm.pkl')
motion2posemodel = joblib.load('new_male_model/male_pose_regression_250.pkl')
...
def load_body_motion():
	...
	sess.run(tf.global_variables_initializer())         # initialize var in graph
	saver = tf.train.Saver()
	save_path = saver.restore(sess, 'new_male_model/deerror1.ckpt')
