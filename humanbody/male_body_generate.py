...
C_body = np.load('DE_training_data.npz')['C']
gmm = joblib.load('gmm.pkl')
motion2posemodel = joblib.load('pose_regression_model.m')
...
def load_body_motion1():
	...
	sess.run(tf.global_variables_initializer())         # initialize var in graph
	saver = tf.train.Saver()
	save_path = saver.restore(sess, 'model/deerror1.ckpt')
