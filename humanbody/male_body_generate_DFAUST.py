...
C_body = np.load('F:\\scan\\MFaust\\scripts\\male\\mixedDE\\male_pose_training_data_all.npz')['C']
gmm = joblib.load('gmm.pkl')
motion2posemodel = joblib.load('mixed_male_deep_network/mixed_male_pose_regression_250.pkl')
...
def load_body_motion():
	...
	sess.run(tf.global_variables_initializer())         # initialize var in graph
	saver = tf.train.Saver()
	save_path = saver.restore(sess, 'mixed_male_deep_network/deerror2.ckpt')
...
def outputPoseParm(x, y):
    ...
    h = motion2posemodel.predict(t).reshape(500)
    return list(h)
